...
static void increment_current_time(effect_t *eff);

static void free_sunrise(effect_t *self) {
...
static int sunrise_run(effect_runner_t *self) {
  int i = (int) self->frame_no;
  if (i >= NUM_PIXELS) {
    i = 0;
    self->frame_no = 0;
    self->effect->get_pixel(self, i);
    increment_current_time(self->effect);
    if (!opc_put_pixel_list(self->sink,
                            self->frame->pixels,
                            self->pixel_info)) {
      return 1;
    }
...
static int parse_time_string(char *time_chars, struct tm *time) {
...
static double get_seconds_for_time(struct tm *time) {
  return (3600 * time->tm_hour) + (60 * time->tm_min) + (time->tm_sec);
}

static double get_time_difference(struct tm *first, struct tm *second) {
...
int sun_get_pixel(effect_runner_t *self, int pos) {
  ...
  struct tm sunrise;
  struct tm sunset;
  if (fgets(buf, (int) buf_size, state->sun_file) == NULL) {
    fseek(state->sun_file, 0, SEEK_SET);
    fgets(buf, (int) buf_size, state->sun_file);
  }
  //printf("%s \n", buf);
  ...
  if (daylight == 0) {
    ...
    if (before_sunrise < 0) {
      ...
      if (before_sunset > 0) {
        ...
        set = (int) ((before_sunset / (daylight / 2.0)) * PIXEL_COLOUR_MAX);
    ...
    if (before_sunset > 0) {
      ...
      double
          before_sunrise = get_time_difference(&sunrise, state->current_time);
      if (before_sunrise < 0) {
        //Current is after sunrise
        daylight += (3600 * 24);
        set = (int) ((-before_sunrise / (daylight / 2.0)) * PIXEL_COLOUR_MAX);
        set = (set > PIXEL_COLOUR_MAX) ? (PIXEL_COLOUR_MAX) - set : set;
        //printf("Set in greyscale: %d",set);
      }
    }
...
// Not sure what this is for - live sunrise data maybe?
static int load_sun_data(list_t *pixel_info) {
  ...
  for (int i = 0; i < NUM_PIXELS; ++i) {
    ...
    size_t buf_size = 600;
    char buff[buf_size];
    if (get_data_for_geolocation(sockfd,
                                 &geoloc,
                                 SUNRISE_HOST,
                                 SUNRISE_PATH,
                                 "0",
